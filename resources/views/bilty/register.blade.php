    ...
    <div class="report-card">
            ...
            <table class="report-table">
                ...
                <tbody>
                    @forelse($bilties as $index => $b)
                        ...
                        @endphp
                        <tr onclick="window.location='{{ route('bilty.create') }}?bilty_no={{ $b->bilty_no }}'"
                            class="{{ $isDraft ? 'draft-row' : '' }}"
                            style="cursor: pointer; {{ $isDraft ? 'background-color: #fef08a;' : '' }}">
                            <td>{{ $index + 1 }}</td>
                            <td>
                                @if($isDraft)
                                    <span
                                        style="background:#fffbeb; color:#92400e; border:1px solid #d97706; padding:2px 8px; border-radius:10px; font-size:10px; font-weight:800; display:inline-block; white-space:nowrap;">Draft</span>
                                @else
                                    <span
                                        style="background:#dcfce7; color:#15803d; border:1px solid #86efac; padding:2px 8px; border-radius:10px; font-size:10px; font-weight:700; display:inline-block; white-space:nowrap;">Final</span>
                                @endif
                            </td>
                            <td><span
                                    style="font-weight:700; color:#0f3460;">{{ $b->series ?: ($b->seriesModel?->name ?? '-') }}</span>
                            </td>
                            <td>{{ $b->bilty_no }}</td>
                            ...
                            <td>{{ $b->consignor ? ($b->consignor->ledger_name ?? $b->consignor->name) : ($b->consignor_name ?? '') }}
                            </td>
                            <td>{{ $b->consignor ? ($b->consignor->mobile ?: ($b->consignor->phone_o ?: '')) : ($b->consignor_mobile ?? '') }}
                            </td>
                            <td>{{ $b->consignee ? ($b->consignee->ledger_name ?? $b->consignee->name) : ($b->consignee_name ?? '') }}
                            </td>
                            <td>{{ $b->consignee ? ($b->consignee->mobile ?: ($b->consignee->phone_o ?: '')) : ($b->consignee_mobile ?? '') }}
                            </td>
                            <td>{{ $b->billingParty ? ($b->billingParty->ledger_name ?? $b->billingParty->name) : ($b->billing_party_name ?? '') }}
                            </td>
                            <td>{{ $b->vehicle_no }}</td>
                            ...
                            <td>
                                ...
                                @if(in_array($shipStatus, ['Shipped', 'In Transit']))
                                    <span
                                        style="background:#e0f2fe; color:#0369a1; border:1px solid #7dd3fc; padding:2px 6px; border-radius:10px; font-size:10px; font-weight:700; white-space:nowrap;">{{ $shipStatus }}</span>
                                @elseif($shipStatus === 'Delivered')
                                    <span
                                        style="background:#dcfce7; color:#15803d; border:1px solid #86efac; padding:2px 6px; border-radius:10px; font-size:10px; font-weight:700; white-space:nowrap;">Delivered</span>
                                @elseif($shipStatus === 'Cancelled')
                                    <span
                                        style="background:#fee2e2; color:#b91c1c; border:1px solid #fca5a5; padding:2px 6px; border-radius:10px; font-size:10px; font-weight:700; white-space:nowrap;">Cancelled</span>
                                @else
                                    <span
                                        style="background:#f1f5f9; color:#475569; border:1px solid #cbd5e1; padding:2px 6px; border-radius:10px; font-size:10px; font-weight:700; white-space:nowrap;">Booked</span>
                                @endif
                            </td>
                            ...
                            <td>{{ $b->user ? ($b->user->username ?? $b->user->name) : ($b->user_id ? 'User #' . $b->user_id : 'admin') }}
                            </td>
                            <td>
                                <a href="{{ route('bilty.print', $b->id) }}" target="_blank" onclick="event.stopPropagation();"
                                    style="color: #003087; font-weight:600; text-decoration:underline;">Print</a>
                                <span style="color:#ccc;">|</span>
                                <a href="{{ route('bilty.pdf', $b->id) }}" target="_blank" onclick="event.stopPropagation();"
                                    style="color: #c0392b; font-weight:600; text-decoration:underline;">PDF</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="34" style="text-align: center; color: #666; padding: 20px;">No Bilty records found for
        ...
        <div class="report-footer">
            ...
            <div class="action-buttons">
                <button type="button" class="btn-action" id="btnPrintList" onclick="downloadExcel(event)"
                    title="Download Excel Sheet of C.N Bills">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                        stroke-linecap="round" stroke-linejoin="round" style="color:#107c41;">
