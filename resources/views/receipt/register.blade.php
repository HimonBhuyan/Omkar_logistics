...
@section('title', 'Receipt Register - Omkaar Logistics')

@section('styles')
<style>
    /* Window Container */
    .receipt-register-window {
        width: 100%;
        max-width: 100%;
        margin: 0 auto;
        background: #d4d0c8;
...
<div class="container-fluid" style="padding: 6px 10px;">
    ...
    <div class="receipt-register-window">
        ...
        <form action="{{ route('receipt.register') }}" method="GET" id="receiptRegisterFilterForm">
            <div class="register-filter-panel">
                ...
                <div class="filter-grid-row">
                    <div class="ctrl-item">
                        ...
                        <select name="series" id="series_input" style="width: 80px; height: 26px; font-size: 12px;">
            ...
            <table class="register-data-table" id="receiptRegisterTable">
                ...
                <tbody>
                    ...
                    @forelse ($receipts as $r)
                        ...
                        @endphp
                        <tr class="data-row" onclick="selectRegisterRow(this)" ondblclick="window.location.href='{{ route('receipt.edit', $r->id) }}'" title="Double click to edit Receipt #{{ $r->series }}-{{ $r->receipt_no }}">
                            <td class="text-center">{{ $srNo++ }}</td>
                            <td class="text-center">{{ $r->series }}</td>
                            ...
                            <td class="text-center">{{ $r->receipt_date ? $r->receipt_date->format('d-m-Y') : '' }}</td>
                            <td class="text-left font-bold" style="max-width: 250px; overflow: hidden; text-overflow: ellipsis;" title="{{ $r->account_name }}">
                                {{ $r->account_name }}
                            </td>
                            <td class="text-right">{{ number_format($tds, 2) }}</td>
                            <td class="text-right font-bold">{{ number_format($amt, 2) }}</td>
                            <td class="text-center font-bold">{{ $r->pay_mode }}</td>
                            <td class="text-left" style="max-width: 180px; overflow: hidden; text-overflow: ellipsis;">
                                {{ $r->bank_name ?: ($r->bank ? $r->bank->ledger_name : '') }}
                            </td>
                            <td class="text-center">{{ $r->cheque_no ?: '' }}</td>
                            <td class="text-center">{{ $r->cheque_date ? $r->cheque_date->format('d-m-Y') : '' }}</td>
                            <td class="text-left" style="max-width: 160px; overflow: hidden; text-overflow: ellipsis;" title="{{ $r->remark }}">
                                {{ $r->remark }}
                            </td>
                            <td class="text-center font-bold" style="white-space: nowrap;">
                                {{ $r->user ? ($r->user->name ?: $r->user->username) : 'ADMIN' }}
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="13" style="text-align: center; color: #666; padding: 40px; font-size: 12px; background: #fff;">
                    ...
                    <tr class="total-summary-row">
                        <td colspan="5" class="text-right font-bold" style="letter-spacing: 0.5px;">
                            Total
                        </td>
                        <td class="text-right font-bold">{{ number_format($totalTdsAmt, 2) }}</td>
                        <td class="text-right font-bold">{{ number_format($totalRectAmt, 2) }}</td>
        ...
        <div class="register-footer-bar">
            <button type="button" class="btn-register-action" onclick="window.print();">
                Print
            </button>

            <a href="{{ route('receipt.register.export', request()->all()) }}" class="btn-register-action" title="Export to Excel">
                Export
            </a>

            <a href="{{ route('dashboard') }}" class="btn-register-action" title="Close">
                Close
            </a>
        </div>
    </div>
...
@section('scripts')
<script>
    function selectRegisterRow(row) {
        document.querySelectorAll('#receiptRegisterTable tr.data-row').forEach(r => r.classList.remove('selected-row'));
